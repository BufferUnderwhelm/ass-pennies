Ass-Pennies Core version 0.13.2 is now available from:

  <https://download.ass-pennies.org/ass-pennies-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a ass-pennies
...
ECDSA signatures inside Ass-Pennies transactions now use validation using
...
Previous versions of Ass-Pennies Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ass-Pennies Core was to raise the default minimum
relay fee.

Ass-Pennies Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ass-Pennies Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ass-penniesd without having to do any manual configuration.
...
Ass-Pennies Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ass-Pennies Core will relay transactions with
...
entirely in Ass-Pennies Core 0.13, and it is at this time undecided whether the
...
Ass-Pennies Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ass-Pennies Core automatically creates a hidden service to listen on, without
manual configuration. Ass-Pennies Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ass-Pennies Core is listening, and
...
Ass-Penniesd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ass-Pennies
Core to use floating fees. By default, Ass-Pennies Core will use floating
fees.
...
(default: `0.0002` ASS/kB).

At all times, Ass-Pennies Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ASS.
Furthermore, Ass-Pennies Core will never create transactions paying less than
...
`ass-pennies.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ass-penniesrpc>
        ...
        # optional bypass ass-penniesd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ass-penniesrpc:<password>
    </Location>
...
Restarting ass-penniesd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ass-pennies.conf
...
ass-pennies-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/ass-pennies-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Ass-Pennies Core code base has
...
- `ass-pennies-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ass-pennies-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Ass-Pennies scripting system that in combination with
...
Ass-Pennies transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Ass-Pennies Core.
...
The two phases are distinct in the Ass-Pennies-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ass-Pennies wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ass-Pennies.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ass-Pennies block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ass-penniess they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ass-penniess being spent is exactly
...
- **Increased security for multisig:** Ass-Pennies addresses (both P2PKH addresses
  ...
  believed to be provided by Ass-Pennies's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ass-Pennies's current ledger (called the UTXO set),
  ...
  note that Ass-Pennies Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ass-Pennies users to individually opt-in to almost any change in the Ass-Pennies
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ass-Pennies Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ass-Pennies
client to Ass-Pennies Core 0.13.2 to signal for segwit support. When a
...
Since Ass-Pennies Core 0.10.0, nodes have defaulted to only relaying and
...
- `ass-pennies-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ass-pennies-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
