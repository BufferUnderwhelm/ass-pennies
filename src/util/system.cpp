...
const char * const BITCOIN_CONF_FILENAME = "ass-pennies.conf";
...
#else
    const char* pszModule = "ass-pennies";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ass-Pennies";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ass-Pennies";
#else
    // Unix
    return pathRet / ".ass-pennies";
#endif
#endif
}
